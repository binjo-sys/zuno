...
export default{async fetch(request,env){if(request.method==="OPTIONS")return json(null);const url=new URL(request.url);try{
...
if(name!==undefined&&name.length<2)return json({error:"Name is too short."},400);if(avatar.length>1500000)return json({error:"Profile photo is too large."},400);if(username!==undefined&&username!==""&&!validUsername(username))return json({error:"Username must be 3-24 characters using letters, numbers or underscores."},400);if(about!==undefined&&about.length>160)return json({error:"About must be 160 characters or fewer."},400);if(username!==undefined&&username!==""&&await env.DB.prepare("SELECT id FROM users WHERE lower(username)=lower(?) AND id<>?").bind(username,a.user.id).first())return json({error:"That username is already taken."},409);
await env.DB.prepare("UPDATE users SET name=COALESCE(?,name),avatar=?,username=?,about=?,updated_at=? WHERE id=?").bind(name===undefined?null:name,avatar,username===undefined?(a.user.username||null):(username||null),about===undefined?(a.user.about||""):about,Date.now(),a.user.id).run();const u=await env.DB.prepare("SELECT id,name,phone,avatar,username,about,last_seen,created_at FROM users WHERE id=?").bind(a.user.id).first();return json({user:u?publicUser(u):null});}
if(url.pathname==="/api/users"&&request.method==="GET"){
const q=url.searchParams.get("q")?.trim().toLowerCase()||"";const rows=q?await env.DB.prepare("SELECT id,name,phone,avatar,username,about,last_seen,created_at FROM users WHERE id<>? AND (lower(name) LIKE ? OR lower(COALESCE(username,'')) LIKE ? OR phone LIKE ?) ORDER BY name LIMIT 100").bind(a.user.id,`%${q}%`,`%${q}%`,`%${q}%`).all():await env.DB.prepare("SELECT id,name,phone,avatar,username,about,last_seen,created_at FROM users WHERE id<>? ORDER BY name LIMIT 100").bind(a.user.id).all();return json({users:rows.results.map(publicUser)});}
if(url.pathname==="/api/messages"&&request.method==="GET"){const withUser=url.searchParams.get("with");if(!withUser)return json({error:"with is required."},400);const rows=await env.DB.prepare("SELECT id,sender_id,recipient_id,body,created_at FROM messages WHERE (sender_id=? AND recipient_id=?) OR (sender_id=? AND recipient_id=?) ORDER BY created_at ASC LIMIT 500").bind(a.user.id,withUser,withUser,a.user.id).all();return json({messages:rows.results});}
if(url.pathname==="/api/messages"&&request.method==="POST"){
const b=await request.json();const recipientId=b.recipientId||"",body=b.body?.trim()||"";if(!recipientId||!body)return json({error:"recipientId and body are required."},400);if(body.length>5000)return json({error:"Message is too long."},400);if(!await env.DB.prepare("SELECT id FROM users WHERE id=?").bind(recipientId).first())return json({error:"User not found."},404);const id=crypto.randomUUID(),createdAt=Date.now();await env.DB.prepare("INSERT INTO messages(id,sender_id,recipient_id,body,created_at) VALUES(?,?,?,?,?)").bind(id,a.user.id,recipientId,body,createdAt).run();const room=env.CHAT_ROOM.idFromName([a.user.id,recipientId].sort().join(":"));await env.CHAT_ROOM.get(room).fetch("https://zuno.internal/broadcast",{method:"POST",body:JSON.stringify({id,senderId:a.user.id,recipientId,body,createdAt})});return json({message:{id,senderId:a.user.id,recipientId,body,createdAt}},201);}
if(url.pathname==="/ws"&&request.headers.get("Upgrade")?.toLowerCase()==="websocket"){const b=url.searchParams.get("b")||"",token=url.searchParams.get("token")||"",aa=await authByToken(token,env);if(!aa)return json({error:"Authentication required."},401);if(!b)return json({error:"b is required"},400);const room=env.CHAT_ROOM.idFromName([aa.user.id,b].sort().join(":"));return env.CHAT_ROOM.get(room).fetch(request);}
return json({error:"Not found"},404);
}catch(e){console.error(String(e));return json({error:"Server error"},500);}}};

export class ChatRoom extends DurableObject{
async fetch(request){if(request.method==="POST"){const message=await request.text();for(const ws of this.ctx.getWebSockets())if(ws.readyState===WebSocket.OPEN)ws.send(message);return new Response("ok");}const pair=new WebSocketPair();const [client,server]=Object.values(pair);this.ctx.acceptWebSocket(server);return new Response(null,{status:101,webSocket:client});}
